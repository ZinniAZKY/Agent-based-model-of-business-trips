from ABM40 import BusinessModel
import pandas as pd
import time
...
def run_business_model(params, model_index, agent_id_offset):
    try:
        ...
        N, admin_grids, agent_freq, ori_type_prob_df, poi_df, startpoi_df, motif_prob, distance_prob, time_prob = params
        model = BusinessModel(N, admin_grids, agent_freq, ori_type_prob_df, poi_df, startpoi_df, motif_prob, distance_prob, time_prob)
        for i in range(1):
            model.step()
...
def run_models_in_parallel(n_models, n_processes):
    ...
    with Pool(n_processes) as pool:
        models = pool.starmap(run_business_model, [(params, i, i * 18591) for i, params in enumerate(params)])
    return models
