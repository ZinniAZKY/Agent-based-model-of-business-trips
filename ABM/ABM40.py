...
class BusinessModel(mesa.Model):
    ...
    def __init__(self, N, admin_grids, agent_freq, ori_type_prob_df, poi_df, startpoi_df, motif_prob, distance_prob, time_prob):
    ...
    def initialize_agent(self, i, type_prob):
        ...
        # oriloc_poi_subset = self.startpoi_df[(self.startpoi_df['DesType'] == a_type) & (self.startpoi_df['x'] == ori_x) & (self.startpoi_df['y'] == ori_y)]
    ...
    # Question: Infinite loops could occur.
    # def filter_buffer_grids(self, dist_x, dist_y):
    #     max_attempts = 500
    #     attempt = 0
    #     sum_desgridprob = 0
    #
    #     while sum_desgridprob == 0 and attempt < max_attempts:
    ...
    #                         break
    #     # Destination choice of agents with number of trips larger than 7
    #     # Question:assign_dest_type may occur infinite loop.
    #     else:
    #         dest_types = [a_type]
    #         for j in range(1, trip_times + 1):
    #             dest_type, x, y, poi_x, poi_y = self.assign_dest_type(a_type, dist_x, dist_y)
    ...
    def process_agent(self, i, type_prob):
        ...
        if 2 <= trip_times <= 7:
            ...
            for j in range(1, trip_times + 1):
                ...
                # (can be improved because agents may move to the same grids but different type of POIs)
                if all(a_pattern[k] != a_pattern[j] for k in range(j)):
                    dest_type, x, y, poi_x, poi_y = self.assign_dest_type(a_type, ori_type, dist_x, dist_y, xlist, ylist)
        ...
        # Destination choice of agents with number of trips larger than 7
            ...
            dest_types = [a_type]
            for j in range(1, trip_times + 1):
                dest_type, x, y, poi_x, poi_y = self.assign_dest_type(a_type, ori_type, dist_x, dist_y)
                if dest_type is None:
