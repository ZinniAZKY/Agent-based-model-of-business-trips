...
class BusinessModel(mesa.Model):
    ...
    def assign_distance(self):
        ...
        if not self.distance_cache:
            for dist_name, prob in zip(self.distance_prob['distance'], self.distance_prob['prob']):self.distance_cache[dist_name] = prob
        dist_name = np.random.choice(list(self.distance_cache.keys()), p=list(self.distance_cache.values()))
    ...
    def assign_pattern(self, trip_times):
        # Check if the result is already in cache
        if trip_times not in self.pattern_cache:
            patternsubset = self.motif_prob.loc[self.motif_prob['tripnum'] == trip_times, 'pattern']
            prob = self.motif_prob.loc[self.motif_prob['tripnum'] == trip_times, 'prob']
            self.pattern_cache[trip_times] = (patternsubset, prob)

        a_pattern = np.random.choice(self.pattern_cache[trip_times][0], p=self.pattern_cache[trip_times][1])
        return a_pattern
    ...
    def create_agents(self):
        ...
        for i in range(self.num_agents):
            ...
            if 2 <= trip_times <= 7:
                ...
                a_pattern_cycle = itertools.cycle(a_pattern)
                for j, pat in zip(range(1, trip_times + 1), a_pattern_cycle):
                    if all(pat != p for p in a_pattern[:j]):
                        dest_type, x, y = self.assign_dest_type(a_type, dist_x, dist_y, xlist, ylist)
