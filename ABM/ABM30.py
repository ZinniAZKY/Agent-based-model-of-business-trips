...
class BusinessModel(mesa.Model):
    ...
    def create_agents(self):
        ...
        all_agents_history = pd.DataFrame(columns=['agent_id', 'x', 'y', 'type', 'oritime'])
        ...
        for i in range(self.num_agents):
            ...
            all_agents_history.loc[len(all_agents_history.index)] = [i, ori_poi_x, ori_poi_y, a_type, random_time]
            ...
            if 2 <= trip_times <= 7:
                ...
                for j in range(1, trip_times + 1):
                    ...
                    if all(a_pattern[k] != a_pattern[j] for k in range(j)):
                        ...
                        all_agents_history.loc[len(all_agents_history.index)] = [i, poi_x, poi_y, a_type, '']
                    else:
