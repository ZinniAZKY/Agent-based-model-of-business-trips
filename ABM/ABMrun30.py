...
def run_business_model(params):
    # Unpack parameters
    N, admin_grids, agent_freq, ori_type_prob_df, poi_df, motif_prob, distance_prob = params

    model = BusinessModel(N, admin_grids, agent_freq, ori_type_prob_df, poi_df, motif_prob, distance_prob)
    for i in range(1):
        model.step()
...
def run_models_in_parallel(n_models, n_processes):
    ...
    with Pool(n_processes) as pool:
        models = pool.map(run_business_model, params)
